...
result = spark.sql("""SELECT education, ROUND(AVG(if(LTRIM(marital_status) = 'Never-married',1,0)),2) as bachelor_rate
  FROM adult
  GROUP BY education
  ORDER BY bachelor_rate DESC""")# fill in here
result.show()
...
df_result = df_adult.select(
  df_adult['education'], 
  when(col('marital_status').contains('Never-married'), 1).otherwise(0).alias('is_bachelor')
)
...
gb.setThresholds([1-training_data_positive_rate, training_data_positive_rate])
...
feature_importance = [(name, float(lrCvModel.bestModel.coefficients[idx])) for idx, name in attrs]
display(pd.DataFrame(feature_importance,columns=['feature_name','feature_importance']).head(10))
